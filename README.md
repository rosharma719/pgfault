...
- **goose**, **Flyway**, and **Alembic** — three more migration tools checked the same way, and the results line up by architecture, not language or popularity: goose and Alembic both commit migration content and version-bookkeeping atomically in one transaction and come through completely clean (self-heal on retry, at worst a wrong error message on the run that actually succeeded). golang-migrate splits those into two transactions on one connection — real lockout. Flyway splits them across **two separate connections** — an ambiguous commit on the one running the actual migration leaves it permanently unrecoverable with a misleading "rolled back" message hiding the real cause, the most severe finding so far. Details for all five, plus root-cause fixes where one is tractable, are tracked in [`case-studies/upstream-fixes.md`](case-studies/upstream-fixes.md).
